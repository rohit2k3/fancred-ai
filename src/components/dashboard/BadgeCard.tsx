...
import { contract, contractAddress } from "@/constant/contact";
import { prepareContractCall } from "thirdweb";

const BadgeCard = ({
  onMintBadgeClick,
}: {
  onMintBadgeClick: () => Promise<void>;
}) => {
  const {
  ...
  React.useEffect(() => {
    if (generatedBadgeArtwork && !isLoadingAiArtwork) {
      // Ensure artwork is present and not loading
      setMintDate(
  ...
  const handleInternalMintClick = async () => {
    if (!isWalletConnected) {
      toast({
        ...
        description: "Please connect your wallet to mint a badge.",
      });
      return;
    }
    ...
    toast({
      title: "Minting Badge...",
      description:
        "Generating AI artwork. This may take a moment. (Mock Mint Fee: 1 CHZ)",
      duration: 15000, // Longer duration for AI generation
    });

    const transaction = mintTo({
      contract,
      to: walletAddress || "", // Use wallet address from context
      nft: {
        name: "NFT Name",
    ...
    try {
      ...
      toast({
        ...
        description: "An unexpected error occurred during minting.",
      });
    } finally {
      setIsMinting(false);
    }
        ...
        {!isMinting && !isLoadingAiArtwork && !generatedBadgeArtwork && (
          <div className="text-center p-4">
            ...
            <p className="text-muted-foreground mb-1">
              Your FanCred Badge isn't minted yet.
            </p>
            ...
            <TransactionButton
              transaction={() =>
                claimTo({
                  contract: contract, // Mock contract address
                  to: walletAddress || "", // Use wallet address from context
                  quantity: BigInt(1),
                })
              }
        ...
        !isMinting && (
          <CardFooter className="flex-col items-start text-sm pt-4 border-t border-white/10">
            <div className="font-semibold text-foreground">
              Level: <span className="text-accent-foreground">{fanLevel}</span>
            </div>
            <div className="text-muted-foreground">Score: {superfanScore}</div>
            <div className="text-muted-foreground">
              Minted: {mintDate} (Mock)
            </div>
          </CardFooter>
        )}
      {(isMinting || isLoadingAiArtwork) &&
        !generatedBadgeArtwork && ( // Show a simplified footer during loading if no badge yet
          <CardFooter className="flex-col items-start text-sm pt-4 border-t border-white/10">
            <div className="text-muted-foreground text-xs">
              Badge details will appear here after generation.
            </div>
          </CardFooter>
        )}
    </Card>
